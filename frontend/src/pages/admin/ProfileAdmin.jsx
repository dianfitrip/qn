...
const ProfileAdmin = () => {
  // --- BASE URL UNTUK FOTO ---
  // Sesuaikan dengan konfigurasi folder statis backend kamu
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      setUserAccount({
        ...
        email: storedUser.email || '-', // Pastikan respons login menyertakan email agar ini tidak strip
  ...
  const handleEditClick = () => {
    setFormData({ ...profile });
    setFotoFile(null);
    setFotoPreview(profile.foto ? `${API_URL}/uploads/${profile.foto}` : null); // Sesuaikan folder statis
    setIsEditing(true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.keys(formData).forEach(key => {
        // Jangan kirim state foto lama agar tidak menimpa jika tidak diganti
        if (key !== 'foto' && formData[key] !== null && formData[key] !== undefined) {
          payload.append(key, formData[key]);
        }
      });

      // Tambahkan file foto baru jika pengguna memilihnya
      if (fotoFile) {
