...
import { ArrowLeft, Plus, Edit2, Trash2, X, Loader2, CheckCircle2, XCircle, Settings, List } from 'lucide-react';

const BankSoalPG = () => {
  // --- STATES ---
  const [loading, setLoading] = useState(false);
  
  // State untuk Tampilan Utama (Daftar Soal)
  const [soalList, setSoalList] = useState([]);
  const [activeSoal, setActiveSoal] = useState(null); // Jika null, tampilkan daftar soal. Jika ada isinya, tampilkan opsi.

  // State untuk Kelola Opsi
  ...
  const fetchSoalList = async () => {
    ...
    try {
      ...
      // Tampilkan semua soal (bisa difilter jika ada tipe soal 'PG' di database Anda)
      const data = res.data.data || res.data;
      setSoalList(data);
    } catch (error) {
      console.error("Error fetching soal:", error);
      Swal.fire('Gagal', 'Tidak dapat mengambil daftar soal', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const openModal = (opsi = null) => {
    if (opsi) {
      setEditingId(opsi.id_opsi); // Sesuai model: id_opsi
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.isi_opsi || !formData.opsi_label) {
      return Swal.fire('Peringatan', 'Label dan isi opsi harus diisi!', 'warning');
    }

    try {
      setLoading(true);
      if (editingId) {
        // UPDATE (PUT /admin/bank-soal-pg/:id)
        await api.put(`/admin/bank-soal-pg/${editingId}`, formData);
        Swal.fire('Berhasil', 'Opsi jawaban berhasil diperbarui', 'success');
      } else {
        // CREATE (POST /admin/bank-soal-pg)
        const payload = {
          ...formData,
          id_soal: activeSoal.id_soal
        };
        await api.post('/admin/bank-soal-pg', payload);
        Swal.fire('Berhasil', 'Opsi jawaban berhasil ditambahkan', 'success');
      }
      closeModal();
      fetchOpsi(activeSoal.id_soal);
    } catch (error) {
  ...
  const handleDelete = async (id_opsi) => {
    const confirm = await Swal.fire({
      title: 'Hapus Opsi?',
      text: "Data opsi ini tidak dapat dikembalikan!",
      icon: 'warning',
    ...
    if (confirm.isConfirmed) {
      try {
        setLoading(true);
        // DELETE (DELETE /admin/bank-soal-pg/:id)
        await api.delete(`/admin/bank-soal-pg/${id_opsi}`);
        Swal.fire('Dihapus!', 'Opsi berhasil dihapus.', 'success');
        fetchOpsi(activeSoal.id_soal);
      } catch (error) {
        Swal.fire('Gagal', 'Gagal menghapus opsi', 'error');
      } finally {
        setLoading(false);
      }
    }
  ...
  // ==========================================
  // RENDER 1: DAFTAR SOAL (PILIH SOAL DULU)
  // ==========================================
  if (!activeSoal) {
    return (
      <div className="p-6 bg-slate-50 min-h-screen">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-black text-[#071E3D]">Pilih Soal Pilihan Ganda</h1>
              <p className="text-slate-500 text-sm">Pilih soal di bawah ini untuk menambahkan opsi jawaban (A, B, C, D).</p>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {soalList.map((soal, idx) => (
              <div key={soal.id_soal || idx} className="bg-white border border-slate-200 rounded-xl p-5 hover:border-[#CC6B27]/50 transition-all shadow-sm flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-[#071E3D] font-medium text-sm line-clamp-3">
                    {soal.pertanyaan || "Teks pertanyaan tidak tersedia..."}
                  </p>
                </div>
                <button 
                ...
                >
                  <Settings size={16} /> Kelola Opsi PG
                </button>
              </div>
            ))}
            {soalList.length === 0 && (
              <div className="col-span-full text-center py-10 text-slate-500">
                Belum ada data Bank Soal. Silakan buat soal terlebih dahulu di menu Bank Soal.
              </div>
            )}
          </div>
        )}
  ...
  // ==========================================
  // RENDER 2: KELOLA OPSI UNTUK SOAL TERPILIH
  // ==========================================
  return (
        ...
        <div className="relative z-10">
          ...
          <div className="bg-white/10 p-4 rounded-xl border border-white/10 mt-4">
            ...
            <p className="text-white text-sm font-medium leading-relaxed">
              {activeSoal.pertanyaan || "Teks pertanyaan..."}
            </p>
          </div>
        </div>
      </div>

      {/* BUTTON TAMBAH & LIST OPSI */}
              ...
              <div 
                ...
                className={`p-4 rounded-xl border-2 transition-all flex flex-col justify-between ${
                  opsi.is_benar 
                    ...
                    : "border-slate-200 bg-white hover:border-[#CC6B27]/50"
                }`}
              >
                <div>
                ...
                <div className="flex justify-end gap-2 pt-3 border-t border-slate-100/50 mt-auto">
                  ...
                  <button
                    ...
                    className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg transition-colors tooltip"
                    title="Hapus"
                  >
                    <Trash2 size={16} />
                  </button>
          ...
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="bg-[#071E3D] p-5 flex justify-between items-center">
              <h2 className="text-white font-bold text-lg flex items-center gap-2">
                ...
                {editingId ? "Edit Opsi Jawaban" : "Tambah Opsi Baru"}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-[13px] font-bold text-[#182D4A] mb-1.5">Label Opsi (A/B/C/D)</label>
                <select
                  name="opsi_label"
                  value={formData.opsi_label}
                  onChange={handleInputChange}
                  className="w-full p-2.5 rounded-lg border border-[#071E3D]/20 focus:border-[#CC6B27] focus:ring-2 focus:ring-[#CC6B27]/10 outline-none text-sm font-medium"
                >
              ...
              <div>
                ...
                <textarea
                  ...
                  required
                  rows="4"
                  className="w-full p-2.5 rounded-lg border border-[#071E3D]/20 focus:border-[#CC6B27] focus:ring-2 focus:ring-[#CC6B27]/10 outline-none text-sm"
              ...
              <div className="flex items-center gap-3 p-3 bg-green-50 border border-green-200 rounded-lg mt-2 cursor-pointer" onClick={() => setFormData(prev => ({...prev, is_benar: !prev.is_benar}))}>
                ...
                <label className="text-[13px] font-bold text-green-800 cursor-pointer select-none flex-1">
                  Jadikan ini sebagai Kunci Jawaban (Benar)
                </label>
              </div>

              <div className="pt-6 flex justify-end gap-3 border-t border-slate-100 mt-6">
                <button type="button" onClick={closeModal} className="px-5 py-2.5 rounded-lg font-bold border border-slate-200 text-slate-600 hover:bg-slate-50 transition-colors text-sm">
                  Batal
                </button>
                <button type="submit" disabled={loading} className="px-5 py-2.5 rounded-lg font-bold bg-[#CC6B27] text-white hover:bg-[#a8561f] shadow-sm flex items-center gap-2 text-sm disabled:opacity-70 disabled:cursor-not-allowed">
